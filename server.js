...
return `<!doctype html><html lang="pl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><link rel="icon" href="/images/favicon.svg"><link rel="stylesheet" href="/css/style.css"></head><body>
...
<header class="header"><a class="logo" href="/"><img src="/images/logo.svg"></a><nav class="menu">
<a class="${active==='home'?'active':''}" href="/">Strona główna</a><a href="/about">O nas</a><a href="/institutions">Dla instytucji</a><a href="/candidates-info">Dla profesjonalistów</a><a href="/academy">Akademia</a><a href="/housing">Mieszkania</a><a href="/poland">Polska</a><a href="/contact">Kontakt</a><a class="portal" href="/login">Portal</a>
</nav></header>${body}</body></html>`;
}
function footer(){return `<footer class="footer"><div><img src="/images/logo.svg"><p>Budujemy mosty między Kolumbią a Polską, łącząc talenty, możliwości i przyszłość.</p></div><div><h4>Szybkie linki</h4><ul><li>Strona główna</li><li>O nas</li><li>Dla instytucji</li><li>Dla profesjonalistów</li></ul></div><div><h4>Polska</h4><p>ul. Prosta 69, Warszawa<br>+48 22 123 45 67<br>info@honorcare.pl</p></div><div><h4>Kolumbia</h4><p>Bogotá<br>+57 320 123 45 67<br>colombia@honorcareinternational.com</p></div></footer>`}
...
app.use(session({secret:SESSION_SECRET,resave:false,saveUninitialized:false,proxy:true,cookie:{httpOnly:true,sameSite:'lax',secure:process.env.NODE_ENV==='production',maxAge:1000*60*60*8},store:MongoStore.create({mongoUrl:MONGO})}));
...
app.get('/', (req,res)=>{
 const body = `<section class="hero"><div class="hero-copy"><h1><span>Kompleksowe</span><span>rozwiązania w opiece zdrowotnej</span><strong>dla Polski</strong></h1><p>Łączymy wykwalifikowanych specjalistów z instytucjami medycznymi w całej Polsce. Rekrutacja, legalizacja, zakwaterowanie i wsparcie — wszystko w jednym miejscu.</p><div class="bullets">${['Rekrutacja międzynarodowa','Legalizacja i dokumenty','Kursy języka polskiego','Wsparcie po przyjeździe','Zakwaterowanie','Integracja i adaptacja','Opieka koordynatora','Dla profesjonalistów i rodzin'].map(x=>`<div><b>✓</b>${x}</div>`).join('')}</div><a class="btn navy" href="/institutions">Dla instytucji</a> <a class="btn gold" href="/candidates-info">Dla profesjonalistów</a> <a class="btn light" href="/contact">▣ Umów spotkanie</a></div><div class="hero-img"></div><aside class="hero-side"><h3>Z miłości do ludzi,<br>z myślą o przyszłości</h3></aside></section>
...
<section class="cards">${[
['img1','DLA PROFESJONALISTÓW','Pracuj w Polsce z nami','Atrakcyjne zarobki','Legalna praca i pełne wsparcie','Pomoc w uznaniu kwalifikacji','ZACZNIJ SWOJĄ PRZYSZŁOŚĆ'],
['img2','DLA INSTYTUCJI','Skuteczne rozwiązania dla Twojej placówki','Selekcja kandydatów','Szkolenia językowe','Stała opieka koordynatora','POPROŚ O OFERTĘ'],
['img3','AKADEMIA HONOR CARE','Szkolenia i rozwój zawodowy','Kursy języka polskiego A1-B2','Terminologia medyczna','Szkolenia online','PRZEJDŹ DO AKADEMII'],
['img4','MIESZKANIA','Komfort i bezpieczeństwo','Nowoczesne mieszkania','Blisko miejsc pracy','Dla rodzin','ZOBACZ MIESZKANIA']
].map(c=>`<article class="card"><div class="img ${c[0]}"></div><div class="card-body"><div class="tag">${c[1]}</div><h3>${c[2]}</h3><ul><li>${c[3]}</li><li>${c[4]}</li><li>${c[5]}</li><li>${c[6]}</li></ul><a class="btn navy small" href="#">${c[6]}</a></div></article>`).join('')}</section>
...
<section class="partners"><h4>NASI PARTNERZY</h4><div><span>NFZ</span><span>PZWL</span><span>NIPiP</span><span>PARPA</span><span>OZZL</span><span>Medicover</span></div></section>${footer()}`;
 res.send(layout('Honor Care International', body));
});

function page(title, text){
 return layout(title, `<section class="page"><h1>${title}</h1><p>${text}</p><div class="grid">${['Proces współpracy','Dokumenty i legalizacja','Szkolenia','Zakwaterowanie','Integracja','Stała opieka'].map(x=>`<article class="panel"><h3>${x}</h3><p>${text}</p></article>`).join('')}</div></section>${footer()}`);
}
app.get('/about',(req,res)=>res.send(page('O nas','Honor Care International łączy Kolumbię i Polskę poprzez profesjonalną rekrutację, edukację i opiekę koordynacyjną.')));
app.get('/institutions',(req,res)=>res.send(page('Dla instytucji','Pomagamy placówkom medycznym w Polsce skutecznie rozwiązać braki kadrowe.')));
app.get('/candidates-info',(req,res)=>res.send(page('Dla profesjonalistów','Tworzymy bezpieczną drogę do pracy w Polsce dla profesjonalistów medycznych z Kolumbii.')));
app.get('/academy',(req,res)=>res.send(page('Akademia','Akademia Honor Care przygotowuje kandydatów językowo, zawodowo i kulturowo.')));
app.get('/housing',(req,res)=>res.send(page('Mieszkania','Organizujemy komfortowe mieszkania i wsparcie integracyjne.')));
app.get('/poland',(req,res)=>res.send(page('Polska','Polska oferuje stabilność, rozwój zawodowy i wysoką jakość życia.')));
app.get('/contact',(req,res)=>res.send(page('Kontakt','Skontaktuj się z nami w sprawie współpracy lub rekrutacji.')));

app.get('/login',(req,res)=>{
 const error = req.query.error ? `<div class="error">${esc(req.query.error)}</div>` : '';
 res.send(layout('Login', `<section class="page"><main class="login"><h1>Beveiligde login</h1><p>Toegang tot documenten en portaal.</p>${error}<form method="post" action="/login"><input name="email" type="email" placeholder="E-mail" required><input name="password" type="password" placeholder="Wachtwoord" required><button class="btn gold full">Inloggen</button></form></main></section>`));
});
...
app.get('/setup-2fa', requireLogin, async (req,res)=>{
 ...
 res.send(layout('2FA', `<section class="page"><main class="login"><h1>Authenticator instellen</h1><img style="max-width:240px" src="${qr}"><p>${secret.base32}</p><form method="post" action="/setup-2fa"><input name="token" placeholder="000000" required><button class="btn gold full">Activeren</button></form></main></section>`));
});
app.post('/setup-2fa', requireLogin, async (req,res)=>{
 ...
 if(!speakeasy.totp.verify({secret,encoding:'base32',token,window:1})) return res.redirect('/setup-2fa');
 await User.findByIdAndUpdate(req.session.userId,{twoFASecret:secret}); req.session.totpPassed=true; res.redirect('/dashboard');
});
app.get('/verify-2fa', requireLogin, (req,res)=>res.send(layout('2FA', `<section class="page"><main class="login"><h1>Authenticator-code</h1><form method="post" action="/verify-2fa"><input name="token" placeholder="000000" required><button class="btn gold full">Verifiëren</button></form></main></section>`)));
...
app.get('/dashboard', requireAuth, async (req,res)=>{
 ...
 res.send(layout('Dashboard', `<section class="page"><div style="display:flex;justify-content:space-between;gap:20px"><div><h1>Dashboard</h1><p>Welkom, ${esc(req.session.email)}.</p></div><form method="post" action="/logout"><button class="btn navy">Uitloggen</button></form></div><section class="modules"><a href="/documents"><b>${counts.docs}</b><span>Documenten</span></a><a href="/candidates"><b>${counts.candidates}</b><span>Kandidaten</span></a><a href="/institutions-list"><b>${counts.institutions}</b><span>Instellingen</span></a><a href="/subsidies"><b>${counts.subsidies}</b><span>Subsidies</span></a></section><h2>Documenten</h2><div class="grid">${docs.map(docCard).join('')}</div></section>`));
});
...
app.get('/documents', requireAuth, async (req,res)=>{
 ...
 res.send(layout('Documenten', `<section class="page"><h1>Documenten</h1><form class="formgrid" method="post" action="/documents"><input name="title" placeholder="Titel"><input name="category" placeholder="Categorie"><input name="language" placeholder="Taal"><input name="status" placeholder="Status"><textarea name="notes" placeholder="Notities"></textarea><textarea name="content" placeholder="Documentinhoud"></textarea><button class="btn gold">Toevoegen</button></form><div class="grid">${docs.map(docCard).join('')}</div><p><a class="btn navy" href="/dashboard">Terug</a></p></section>`));
});
app.get('/test-documents', async (req,res)=>{
 const docs=await Document.find().sort({createdAt:-1}).lean();
 res.send(layout('Test documenten', `<section class="page"><h1>Test documenten zonder login</h1><p>Als je deze ziet, staan documenten correct in MongoDB.</p><div class="grid">${docs.map(d=>`<article class="doc"><h3>${esc(d.title)}</h3><p>${esc(d.category)} • ${esc(d.language)} • ${esc(d.status)}</p><a class="btn navy small" href="/test-documents/${d._id}">Open test</a></article>`).join('')}</div></section>`));
});
app.get('/test-documents/:id', async (req,res)=>{
 ...
 res.send(layout(doc.title, `<section class="page"><a class="btn navy" href="/test-documents">Terug</a><article class="doc-content"><p class="meta">${esc(doc.category)} • ${esc(doc.language)} • ${esc(doc.status)}</p>${docContent(doc)}</article></section>`));
});
app.get('/documents/:id', requireAuth, async (req,res)=>{
 ...
 res.send(layout(doc.title, `<section class="page"><a class="btn navy" href="/documents">Terug</a><article class="doc-content"><p class="meta">${esc(doc.category)} • ${esc(doc.language)} • ${esc(doc.status)}</p>${docContent(doc)}</article></section>`));
});
...
function crud(url,Model,title,type,fields){
 app.get(url,requireAuth,async(req,res)=>{
  const items=await Model.find().sort({createdAt:-1}).lean();
  const form=`<form class="formgrid" method="post" action="${url}">${fields.map(f=>`<input name="${f}" placeholder="${f}">`).join('')}<button class="btn gold">Toevoegen</button></form>`;
  ...
  res.send(layout(title, `<section class="page"><h1>${title}</h1>${form}${list}<p><a class="btn navy" href="/dashboard">Terug</a></p></section>`));
 });
 app.post(url,requireAuth,async(req,res)=>{await Model.create(req.body);res.redirect(url)});
}
crud('/candidates',Candidate,'Kandidaten','candidates',['name','email','profession','country','status','notes']);
crud('/institutions-list',Institution,'Instellingen','institutions-list',['name','contact','email','city','demand','status']);
crud('/subsidies',Subsidy,'Subsidies','subsidies',['title','program','deadline','status','notes']);

mongoose.connect(MONGO).then(async()=>{console.log('MongoDB verbonden'); await seed(); app.listen(PORT,()=>console.log('HonorCare Working Docs v16 draait op poort '+PORT));}).catch(e=>{console.error(e);process.exit(1)});
